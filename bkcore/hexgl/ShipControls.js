	...
	if (ctx.controlType == 1 && bkcore.controllers.TouchController.isCompatible()) {
		...
		function updateInfo() {
			if (!isServerConnected) {
				leapInfo.innerHTML = '正在等待 Leap Motion 控制器服务器...'
				...
				leapInfo.innerHTML = '请连接您的 Leap Motion 控制器。'
				leapInfo.style.display = 'block';
			}
			else if (!lb.hasHands) {
				leapInfo.innerHTML = '请将手放在 Leap Motion 控制器上方进行操控。'
				leapInfo.style.display = 'block';
			}
		}
		updateInfo();
...
bkcore.hexgl.ShipControls.prototype.update = function (dt) {
	...
	if (this.active) {
		...
		if (window.CameraInput && window.CameraInput.active) {
			var ci = window.CameraInput.controls;
			// Apply gesture steering directly (CameraInput handles smoothing and calibration)
			var steerValue = (ci.moveLeft - ci.moveRight);
			angularAmount += steerValue * this.angularSpeed * dt;
			rollAmount -= steerValue * this.rollAngle;

			this.key.forward = ci.accelerate;
			this.key.ltrigger = ci.brake;
			this.key.rtrigger = ci.brake;
		}
		else if (this.touchController != null) {
