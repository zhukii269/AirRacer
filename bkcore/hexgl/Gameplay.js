...
bkcore.hexgl.Gameplay = function (opts) {
	...
	this.modes.timeattack = function () {
		...
		// Only count checkpoint if it's the next one in sequence
		if (cp != -1 && cp != self.previousCheckPoint) {
			var checkpointList = self.track.checkpoints.list;
			var currentIndex = checkpointList.indexOf(self.previousCheckPoint);
			var nextIndex = (currentIndex + 1) % checkpointList.length;
			var expectedNext = checkpointList[nextIndex];

			// Check if this is the expected next checkpoint (going forward)
			if (cp == expectedNext) {
				self.previousCheckPoint = cp;

				// Completed a lap (went from last checkpoint back to start)
				if (cp == self.track.checkpoints.start && currentIndex == checkpointList.length - 1) {
			...
			// Going wrong way - ignore (don't update previousCheckPoint)
		}
